...
class WoopraFrontend extends Woopra {
	 ...
	 */
	 function track_signup($user_id) {
		...
		$this->woopra->identify($identify_user);
		// then track them
		$this->woopra->track('wp_signup', $user_details, true);
	}
	...
	function track_login( $user_login, $user ) {
		...
		$user_details['url']= site_url();

		// get avatar image
		$avatarimg=get_avatar_url($user->user_email, array('size' => 80, 'default' => '404'));

		if (preg_match('/&d=404/', $avatarimg) ) {
			$avatarimg='';
		}
		...
		// tag the user via server push
		$this->woopra->identify($identify_user);

		$this->woopra->track('wp_login', $user_details, true);
	}
